...
void ExportEmbeddedTexture(const aiTexture* texture, const Arguements& args,
	const std::filesystem::path& fileName)
{
	...
	ASSERT(written = texture->mWidth, "Error: fail to extract embedded texture!");
	fclose(file);
}

std::string FindTexture(const aiScene* scene, const aiMaterial* aiMaterial,
	aiTextureType textureType, const Arguements& args, const std::string& suffix,
	uint32_t materialIndex)
{
	...
	if (aiMaterial->GetTexture(textureType, 0, &texturePath) == aiReturn_SUCCESS)
	{
		if (texturePath.C_Str()[0] == '*')
		{
			...
			fileName.reserve(fileName.length() - 4);
			fileName += suffix;
